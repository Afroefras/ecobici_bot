...
class EcoBiciMap:
    ...
    def transform(self, station_cols: list=['id','zipCode','location.lat','location.lon'], id_col: str='id', status_col: str='status', bikes_col: str='availability.bikes', slots_col: str='availability.slots') -> None:
        ...
        self.df['disponible'] = self.df[bikes_col] / (self.df[slots_col] + self.df[bikes_col])
    ...
    def plot_map(self, lat_col: str='location.lat', lon_col: str='location.lon', padding: float=0.007) -> None:
        ...
        self.gdf.plot(ax=ax, linewidth=0.5, color='#F4F6F7', edgecolor='gray')

        cmap = get_cmap('crest')
        scatterplot(y=lat_col, x=lon_col, hue='ocupado', data=self.df, ax=ax, palette=cmap)

        self.set_custom_legend(ax, cmap, values=[(0.0, 'Vacío'), (0.5, 'Ocupado'), (1.0, 'Lleno')])
